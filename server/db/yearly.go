...
func (db *DBDriver) GetYearly(ctx context.Context, m *Yearly) ([]*Yearly, error) {
	where := []string{"1=1"}
	args := []any{}
	...
	sql := `
		...
		from yearly where ` + strings.Join(where, " and ") + `
...
func (db *DBDriver) UpsertYearly(ctx context.Context, m *Yearly) (*Yearly, error) {
	ifields := []string{"uid", "day", "content"}
	placeholder := []string{"?", "?", "?"}
	ufields := []string{"content = ?", "update_time = ?"}
	args := []any{m.Uid, m.Day, m.Content, m.Content, time.Now().Unix()}

	sql := `insert into yearly (` + strings.Join(ifields, ", ") + `) values (` + strings.Join(placeholder, ", ") + `)
